...
pub fn simulate_lwod_generation(grid: &Array2D<i32>, grid_size: usize) -> (Array2D<i32>, i32, i32) {
    ...
    for row in 0..grid_size {
        for col in 0..grid_size {
            ...
            if curr_cell == &0 && alive_neighbors == 3 { //If the cell is dead and has exactly 3 alive neighbors, it comes to life.
                new_grid.set(row, col, 1).ok();
                births += 1;
            } else { //In any other scenario, the cell remains in its current state (either alive or dead). In Life Without Death, once a cell is alive, it never dies.
