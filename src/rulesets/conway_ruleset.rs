...
pub fn simulate_conway_generation(grid: &Array2D<i32>, grid_size: usize) -> (Array2D<i32>, i32, i32) {
    ...
    for row in 0..grid_size {
        for col in 0..grid_size {
            ...
            if curr_cell == &1 && (alive_neighbors < 2 || alive_neighbors > 3)  { //If the cell is alive and is isolated/overpopulated
                new_grid.set(row, col, 0).ok(); //it dies.
                deaths += 1;
            } else if curr_cell == &0 && alive_neighbors == 3  { //If the cell is dead and has exactly 3 alive neighbors
